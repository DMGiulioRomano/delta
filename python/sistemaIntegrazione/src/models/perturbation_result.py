...
class PerturbationResult:
    ...
    def summarize(self) -> str:
        ...
        for param, change in self.applied_changes.items():
            delta_info = f" (Δ: {change['delta']})" if change.get('delta') is not None else ""
            summary.append(
                f"- {param}: {change['old']} → {change['new']}{delta_info}"
            )
...
def create_failed_perturbation(
    state: StatoMusicale,
    error_message: str = "Errore durante la perturbazione"
) -> PerturbationResult:
    """
